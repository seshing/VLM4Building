...
def process_images(gpu_id, image_files, model_name, image_directory, output_directory, city): 
    ...
    for image_name in tqdm(image_files, desc=f"Processing images for city {} on GPU {gpu_id}"):
...
def main():
    ...
    cities = ['Berlin_center', 'Manila_center', 'Berlin_outer', 'Manila_outer']
    for city in cities: 
        image_directory = f'VLM4Building/img/{city}'
        model_output_name = model_name.split('/')[-1]
        output_directory = f'VLM4Building/img/{city}'
        os.makedirs(output_directory, exist_ok=True)

        test_df = pd.read_csv(f'VLM4Building/data/selected_img_id/{city}.csv')
        ...
        for i in range(num_gpus):
            p = multiprocessing.Process(target=process_images, args=(i, image_files_split[i], model_name, image_directory, output_directory, city))
            processes.append(p)
            p.start()
